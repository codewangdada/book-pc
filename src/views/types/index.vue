        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="图书类型" prop="bookType">
              <el-input v-model="form.bookType" placeholder="请输入图书类型" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
import { list, add, update, del } from "@/api/types";
export default {
  data() {
    return {
      total: 10,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      list(this.queryParams,this.queryParams.pageNum,this.queryParams.pageSize).then((response) => {
